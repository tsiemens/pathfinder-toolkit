...
public class CampaignParty implements Parcelable, Storable {
	...
	private ArrayList<PartyMember> m_partyMembers;
	...
	public CampaignParty(long id, String name) {
		this(id, name, new PartyMember[0]);
	}
	
	public CampaignParty(long id, String name, PartyMember[] partyMembers) {
		m_id = id;
		...
		m_partyMembers = new ArrayList<PartyMember>(Arrays.asList(partyMembers));
	}
	 ...
	 */
	public int addPartyMember(PartyMember newPartyMember){
	 ...
	 */
	public int setPartyMember(int index, PartyMember partyMember){
	...
	public String[] getNamesByRollValue(){
		PartyMember[] orderedMembers = getPartyMembersByRollValue();
		String[] names = new String[m_partyMembers.size()];
	 ...
	 * If there is a tie, sorts by default order
	 */
	private PartyMember[] getPartyMembersByRollValue(){
		ArrayList<PartyMember> tempMembers = cloneMemberList();
		PartyMember[] sortedMembers = new PartyMember[m_partyMembers.size()];
		...
		for(int currentNamesIndex = 0; currentNamesIndex < m_partyMembers.size(); currentNamesIndex++){
			...
			sortedMembers[currentNamesIndex] = new PartyMember(tempMembers.get(indexWithLargestRoll));
	 ...
	 */
	private ArrayList<PartyMember> cloneMemberList(){
		int numberOfMembers = m_partyMembers.size();
		ArrayList<PartyMember> membersListClone = new ArrayList<PartyMember>();
		
		for(int i = 0; i < numberOfMembers; i++){
			PartyMember tempMember = new PartyMember(m_partyMembers.get(i));
	...
	public void setID(long id) {
		m_id = id;
		for (PartyMember member : m_partyMembers) {
